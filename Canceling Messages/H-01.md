...
The POC checks that the Bridge will end being the owner of the tokens, and they are not escrowed, which meanas anyone tries to withdraw it back when bridging from `L2` to `L1` will end up trying minting it which will revert.

## Tools Used
Manual Review + Foundry

## Recommendations
Implement CEI patter by burn The token before transfering it.
